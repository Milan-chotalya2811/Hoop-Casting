...
export default function EditProfile() {
    ...
    const [cropFieldName, setCropFieldName] = useState<string | null>(null)
    ...
    const [profile, setProfile] = useState<any>({
        ...
        height_cm: '', weight_kg: '',
        profile_photo_url: '', profile_picture: '', gallery_urls: [], 
    ...
    useEffect(() => {
        if (user) {
            setProfile((prev: any) => ({ ...prev, name: user.name || '', email: user.email || '', whatsapp_number: user.mobile || '' }))
    ...
    const handleFileUpload = async (e: any, fieldName: string) => {
        ...
        if (fieldName === 'profile_picture' || fieldName === 'profile_photo_url') {
            const file = files[0]
    ...
    const handleCropComplete = async (croppedFile: File) => {
        ...
        try {
            setSubmitting(true); setIsUploading(true)
            setFieldMessages(prev => ({ ...prev, profile_picture: { text: 'Syncing DB...', type: 'info' } }))

            const compressedFile = await compressImage(croppedFile) as File
            ...
            setProfile((prev: any) => ({ ...prev, profile_picture: base64data }))
            
            await api.post('/profile.php', { ...profile, profile_picture: base64data, content_rights_agreed: profile.content_rights_agreed ? 1 : 0 })
            await refreshAuth()
            setFieldMessages(prev => ({ ...prev, profile_picture: { text: 'Uploaded!', type: 'success' } }))
        } catch (err: any) {
            setFieldMessages(prev => ({ ...prev, profile_picture: { text: 'Error: ' + err.message, type: 'error' } }))
        } finally { 
            setSubmitting(false); setIsUploading(false); setCropFieldName(null)
        }
    }
        ...
        <div className="container section" style={{ paddingTop: '140px' }}>
                    ...
                    <AnimatePresence mode="wait">
                        {profile.category && (
                            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                                ...
                                <div className="grid">
                                    <Field label="Full Name" value={profile.name} onChange={(e: any) => setProfile({...profile, name: e.target.value})} required />
                                ...
                                <ProfilePhotoSection url={profile.profile_picture || profile.profile_photo_url} onUpload={(e: any) => handleFileUpload(e, 'profile_picture')} status={fieldMessages.profile_picture} />
            ...
            {cropImageSrc && (
                <ImageCropperModal
                    ...
                    onCancel={() => { setCropImageSrc(null); setCropFieldName(null) }}
                />
            )}
    ...
    return (
        <div style={{ margin: '30px 0', padding: '25px', background: '#fdfdfd', border: '1px solid #000', borderRadius: '20px', display: 'flex', gap: '25px', alignItems: 'center' }}>
            <div style={{ position: 'relative' }}>
                <img key={url ? url.substring(0, 100) : 'none'} src={isBase64 ? url : fixUrl(url)} onError={(e: any) => e.target.src = '/default_avatar.png'} style={{ width: 110, height: 140, borderRadius: '15px', objectFit: 'cover', objectPosition: 'top center', border: '2px solid var(--primary)' }} />
