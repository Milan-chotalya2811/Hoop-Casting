...
if ($method == 'GET') {
    ...
    if ($stmt->execute($params)) {
        ...
        // Sync core user details (name and mobile) to the users table
        if (isset($data['name']) && !empty(trim($data['name']))) {
            $updateUser = "UPDATE users SET name = :name, mobile = :mobile WHERE id = :user_id";
            $uStmt = $db->prepare($updateUser);
            
            $name_clean = trim($data['name']);
            $uStmt->bindParam(':name', $name_clean);
            
            $mobile = isset($data['whatsapp_number']) && !empty($data['whatsapp_number']) ? $data['whatsapp_number'] : ($user['mobile'] ?? '');
            $uStmt->bindParam(':mobile', $mobile);
            $uStmt->bindParam(':user_id', $user['id']);
            $uStmt->execute();
        }
